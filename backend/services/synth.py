...
REASON_RULES: list[dict[str, str]] = [
    ...
    {"id": "resource", "label": "资源获取",
     "tmpl": "能对上这个方向的资料里，最贴的一条是：{e1}。\n"
             ...
             "{infer}"},
    # v7.5 新增（加规则只 append）：学生侧「上传解析 / 总结」类意图的综合模板。
    {"id": "material", "label": "资料解析",
     "tmpl": "这份材料的核心内容可以先概括为：{e1}。\n"
             "再往下一层，{e2}，这两块合起来就是它的主干。\n"
             "{infer}"},
...
INFER_RULES: dict[str, str] = {
    ...
    "material": "总结不是压缩字数：先把主干抽出来，再决定哪些细节值得留，解析结果才能当提纲用。",
...
REMIND_RULES: dict[str, str] = {
    ...
    "material": "解析和总结都只覆盖这份材料本身：材料之外的延伸我会标明是推理，别当成原文。",
