...
│       └── tasks.py        成长任务
├── frontend/
│   ├── css/style.css     单一设计系统（409 个 class，无框架）
...
| 类别 | 说明 |
|---|---|
| 品牌色 | 亮天蓝 `--brand-500:#16a1ff`（HSL 203,100%,54%），9 级色阶 50→800；v2.3 从 `#3a72c4` 换到 `#16a1ff`，饱和更高、更亮，彻底去灰 |
| 副强调 | 蓝紫 `--accent-500:#5b8def`，只用于第二层级信息与「学业型」轨道 |
...
### UI/UX v2.3：主色换成 #16A1FF + 区块描边

用户反馈「太灰、各方块分不清」。这一轮只换颜色与描边，**栅格、尺寸、间距一行没动**。

| 维度 | 改动 |
|---|---|
| 主色 | `--brand-500` `#3a72c4` → **`#16a1ff`**，整套 50→800 色阶重画；暗色主题同步换成亮蓝同源色（`#4aa8f0`） |
| 副色 | 淡靛 `#6b78c9` → 蓝紫 `#5b8def`，与主色同一呼吸 |
| 去灰 | `--bg` → `#f1f7ff`、`--border` → `#dbe7f7`、`--border-strong` → `#a8ccee`，底色与分隔线都带蓝调 |
| 硬编码 | 全站 30 处 `rgba(58,114,196,…)` → `rgba(22,161,255,…)`；暗色 12 处 `rgba(92,142,201,…)` → `rgba(74,168,240,…)` |
...
- 网格点：从 `rgba(27,36,55,.06)` 灰点改成 `rgba(22,161,255,.26)` 蓝点、20px 间距、遮罩放宽到 135%×95%，「图纸感」更明确但仍压得住内容；暗色下换成 `rgba(124,178,240,.20)`。
- 品牌光带：顶部新增 `.shell::before` 一条 **2px 品牌渐隐轨**（`--brand-300 → --brand-500 → #7cc2f7 → 透明`），一进页面就有品牌识别。
- 光晕：三团 radial 重构成从左上到右下的「天光」，页面有了方向感。
...
| 组件 | 状态补强 |
|---|---|
| 按钮 | 焦点环统一成品牌色 `0 0 0 3px rgba(22,161,255,.22)`；主按钮 focus 叠加品牌投影；disabled 降饱和到 `.65`；ghost hover 转品牌色 |
...
| 浏览器实测（`getComputedStyle`） | 网格点 `rgba(22,161,255,.26)`、卡片描边 `#a8ccee`、卡片头竖条 `3px`、顶部品牌轨 `2px`、进场动画 `pf-rise`、普通分布格描边 `#9dd9ff`、说明条底 `#e9f6ff` —— 全部命中 |
