...
界面经过多轮 UI/UX 重构，目标是「成熟 SaaS 产品的观感」，约束是**不碰任何业务逻辑**。

### 设计令牌（`frontend/css/style.css` 顶部 `:root`）

| 类别 | 说明 |
|---|---|
| 品牌色 | 电光蓝 `--brand-500:#2563eb`，9 级色阶 50→800。v3 由晴空蓝 `#3a72c4` 转来：饱和度抬到 85% 上下，屏幕上一眼可辨，但仍是可信的企业蓝而非霓虹 |
| 副强调 | 靛蓝 `--accent-500:#6366f1`，用于第二层级信息与「学业型」轨道 |
| 语义色 | `--ok-500:#10b981 / --warn-500:#f59e0b / --danger-500:#ef4444 / --info-500:#0ea5e9`，提到「一眼能认出」的饱和度，四色明度齐平不互相抢戏 |
| 中性色 | 中性炭黑灰阶 12 级 `--ink-950…--ink-50`（950 `#0f172a` → 50 `#f9fafb`）。v3 起**不再掺蓝** —— 旧色阶每档都掺蓝，叠在蓝底上整页发闷，是「发灰」的主因 |
| 表面 | `--bg:#f8fafc`（近纯白）/ `--surface:#fff` / `--surface-2` / `--surface-3` / `--surface-glass`。底色与卡片只差一档，靠**描边与留白**分层，不再靠把阴影压黑 |
| 圆角 | `--r-xs:6 → --r-xl:22 → --r-pill:999`，6 级 |
| 阴影 | `--sh-1 → --sh-4`，统一中性黑 `rgba(15,23,42,*)`、不透明度 3%~11%。蓝调阴影叠白底会让卡片边缘发脏，是「发灰」的第二来源 |
| 字号 | 6 级：`--fs-display:34 / --fs-h1:25 / --fs-h2:19 / --fs-h3:16 / --fs-body:14 / --fs-sm:13 / --fs-xs:12`。v3 把标题档拉开，标题与正文差距 ≥5px，一屏能分清「看什么/读什么/扫什么」 |
